...
class PaymentSession(Base):
    ...
    # Direct Tabby payment id (when Bitrix mode is Tabby; HPP URL still in paymob_checkout_url)
    tabby_payment_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
    # Direct Tamara order id (when Bitrix mode is Tamara; HPP URL still in paymob_checkout_url)
    tamara_order_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
